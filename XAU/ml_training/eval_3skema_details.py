...
def run_both_simulations(data, min_prob, step_pts=400, max_marti=6, tp_usd=3.0, base_lot=0.01, mult=2.0, point=0.01):
    opens = data['open'].values
    highs = data['high'].values
    lows = data['low'].values
    closes = data['close'].values
    atrs = data['atr_normalized'].values * data['close'].values / 1000.0
    ...
    # Model A: Full Recovery Basket Martingale (SL awal tidak mematikan grid, basket trailing menutup grup)
    cycles = []
    i = 0
    while i < N - 1:
    ...
    print(f"Total Siklus: {len(cdf)}")
    print(f"Level Terdalam: Level {cdf['max_level'].max()}")
    print(f"Level 0 (Tanpa Averaging): {counts.get(0, 0)} ({counts.get(0, 0)/len(cdf)*100:.2f}%)")
    print(f"Level 1: {counts.get(1, 0)} ({counts.get(1, 0)/len(cdf)*100:.2f}%)")
    print(f"Level 2: {counts.get(2, 0)} ({counts.get(2, 0)/len(cdf)*100:.2f}%)")
    print(f"Level 3: {counts.get(3, 0)} ({counts.get(3, 0)/len(cdf)*100:.2f}%)")
    print(f"Level 4: {counts.get(4, 0)} ({counts.get(4, 0)/len(cdf)*100:.2f}%)")
    print(f"Level 5: {counts.get(5, 0)} ({counts.get(5, 0)/len(cdf)*100:.2f}%)")
    print(f"Level 6 (Max Step Terkena): {counts.get(6, 0)} ({counts.get(6, 0)/len(cdf)*100:.2f}%)")
    print(f"Rata-rata Drawdown per Siklus: ${cdf['max_dd'].mean():.2f}")
    print(f"Median Drawdown per Siklus: ${cdf['max_dd'].median():.2f}")
    print(f"P95 Drawdown (95% Siklus di bawah): ${cdf['max_dd'].quantile(0.95):.2f}")
    print(f"Max Floating Drawdown: ${cdf['max_dd'].max():.2f}\n")
    return cdf

cdf_a = run_both_simulations(df, 0.35)
cdf_b = run_both_simulations(df, 0.45)
cdf_c = run_both_simulations(df, 0.55)
