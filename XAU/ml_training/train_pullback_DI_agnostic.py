"""
Script Training Machine Learning DI-AGNOSTIC untuk EA XAUUSD PullBack & Reversal Pullback Engine
Copy dari train_pullback_model.py — perbedaan hanya sumber dataset & nama output ONNX.
...
Output         : model_xau_pullback_DI.onnx (19 fitur, skema & urutan sama -> kompatibel EA)

Model ini belajar dari sinyal searah DI DAN melawan DI (dengan simulasi outcome
eksekusi pullback/discount), sehingga probabilitas win bermakna untuk SEMUA
segmen sinyal (menutup Out-Of-Distribution gap pada EA PullBack).

Hyperparameter & logika simulasi pullback identik dengan train_pullback_model.py
agar perbandingan model lama vs baru murni disebabkan perbedaan dataset.
"""
...
def load_data(csv_path: str):
    ...
    if not os.path.exists(csv_path):
        ...
        print("Pastikan file xauusd_ml_dataset_di.csv ada di direktori kerja.")
        sys.exit(1)
...
def train_and_export(df: pd.DataFrame, output_onnx: str = "model_xau_pullback_DI.onnx"):
    ...
    # --- XGBoost Classifier (hyperparameter identik dengan train_pullback_model.py) ---
    model = XGBClassifier(
        n_estimators=120,
        max_depth=4,
        learning_rate=0.03,
    ...
    print("\nMelatih model XGBoost PullBack DI-Agnostic...")
    ...
    print("\n=== EVALUASI MODEL PULLBACK DI-AGNOSTIC (threshold=0.50) ===")
    ...
    print(f"\n=== EVALUASI MODEL PULLBACK DI-AGNOSTIC (threshold={best_thr:.3f} - OPTIMAL) ===")
    ...
    print("\n=== TOP FEATURE IMPORTANCE PULLBACK DI-AGNOSTIC (XGBoost gain) ===")
