//+------------------------------------------------------------------+
//|                                  Export_ML_Dataset_DI_Agnostic.mq5|
//|     Script Ekspor Fitur & Labeling Historis untuk ML/ONNX         |
//|                        VERSI DI-AGNOSTIC (tanpa filter arah DI)   |
//|               Mendukung XAUUSD / XAUUSD.vx (Semua Timeframe)      |
//+------------------------------------------------------------------+
//| PERBEDAAN vs Export_ML_Dataset.mq5 (v2.14):                       |
//|  1. Sinyal FOLLOW tidak lagi mensyaratkan arah DI (+DI/-DI).      |
//|     -> Sinyal "melawan DI" ikut terekam, agar model ML belajar    |
//|        dari semua segmen (menutup Out-Of-Distribution gap).       |
//|     -> Keputusan filter DI diserahkan ke EA (InpADX_UseDI).       |
//|  2. Parameter deteksi diselaraskan dengan file .set live:         |
//|     ADX >= 15, RSI 65/35 (script lama: ADX >= 20, RSI 70/30).     |
//|  3. Skema 19 fitur + label_win TIDAK berubah -> kompatibel penuh  |
//|     dengan kode EA & semua script simulasi yang ada.              |
//+------------------------------------------------------------------+
#property copyright   "XAU Phase1 & Phase2 - DI Agnostic"
#property link        ""
...
input string             InpFileName      = "xauusd_ml_dataset_di.csv"; // Nama file output (di folder MQL5/Files)

//--- Parameter Indikator (selaras dengan file .set live EA)
input int                InpEMA_Period    = 50;
input int                InpADX_Period    = 14;
input int                InpADX_Strength  = 15;       // Min ADX (selaras .set: InpADX_Strength=15)
input int                InpRSI_Period    = 14;
input double             InpRSI_Overbought= 65.0;     // RSI overbought (selaras .set: 65)
input double             InpRSI_Oversold  = 35.0;     // RSI oversold (selaras .set: 35)
...
void OnStart()
  {
   ...
   Print("Memulai ekspor data ML DI-AGNOSTIC untuk: ", sym, " Timeframe: ", EnumToString(InpTimeframe));
   ...
   //--- Tulis Header CSV (19 Fitur + Target) -- SKEMA SAMA DENGAN SCRIPT LAMA
   ...
   for(int i = copied_rates - 100; i >= horizon; i--)
     {
      ...
      //=================================================================
      //=== Deteksi Tipe Sinyal Kandidat (DI-AGNOSTIC)                 ===
      //=== TANPA syarat arah DI. Parameter selaras .set live:        ===
      //=== ADX >= 15, RSI 65/35. Arah DI tetap terekam sebagai fitur ===
      //=== (adx_pdi / adx_mdi / adx_di_diff) agar model ML belajar   ===
      //=== dari sinyal searah DAN melawan DI.                        ===
      //=================================================================
      int signal_type = 0;
      // Follow Buy (tanpa syarat DI — keputusan UseDI ada di EA)
      if(cur_close > ema[i] && adx_m >= InpADX_Strength && cur_rsi < InpRSI_Overbought) signal_type = 1;
      // Follow Sell
      else if(cur_close < ema[i] && adx_m >= InpADX_Strength && cur_rsi > InpRSI_Oversold) signal_type = 2;
      // Reversal Buy (RSI oversold + Pinbar/Engulfing)
      else if(cur_rsi <= InpRSI_Oversold && (is_pinbar == 1 || is_engulfing == 1)) signal_type = 1;
      // Reversal Sell (RSI overbought + Pinbar/Engulfing)
      else if(cur_rsi >= InpRSI_Overbought && (is_pinbar == -1 || is_engulfing == -1)) signal_type = 2;
   ...
   Print("Ekspor DI-AGNOSTIC selesai! Berhasil menyimpan ", valid_rows, " baris data ke file: MQL5/Files/", InpFileName);
  }
//+------------------------------------------------------------------+
