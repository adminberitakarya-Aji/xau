...
#resource "model_xau_DI.onnx" as uchar ExtModelONNX[]
...
enum ENUM_TRADE_MODE
  {
   MODE_FOLLOW_ONLY,   // 0: Hanya Inverse dari FOLLOW tren
   MODE_REVERSAL_ONLY, // 1: Hanya Inverse dari Pola REVERSAL
...
input int                InpMagicNumber   = 28282829;      // Magic number (Reversal Engine)
...
void UpdateDashboard()
  {
   ...
   if(buffers_ready)
     {
      ema_trend   = (iClose(trade_symbol, _Period, 1) > ema_value[1]) ? "BULLISH [Harga > EMA] (Reversal Target: SELL)" : "BEARISH [Harga < EMA] (Reversal Target: BUY)";
...
int OnInit()
  {
   ...
   if(!sym_ok)
     {
      Alert("[XAU EA REVERSAL] Gagal memilih simbol: ", trade_symbol, ". Pastikan pair ada di Market Watch!");
   ...
   if(handle_EMA==INVALID_HANDLE || handle_EMA_H1==INVALID_HANDLE || handle_ADX==INVALID_HANDLE ||
     ...
     {
      Alert("[XAU EA REVERSAL] Gagal buat handle indikator untuk pair: ", trade_symbol);
   ...
   if(InpUseMLFilter)
     {
      ...
      if(handle_onnx == INVALID_HANDLE)
         handle_onnx = OnnxCreate("model_xau_DI.onnx", ONNX_DEFAULT);
      
      //--- 3. Fallback: coba dari subfolder Experts
      if(handle_onnx == INVALID_HANDLE)
         handle_onnx = OnnxCreate("Experts\\model_xau_DI.onnx", ONNX_DEFAULT);
...
//+------------------------------------------------------------------+
//| Deteksi sinyal entry: REVERSAL ENGINE (Kebalikan dari Phase 1)   |
//+------------------------------------------------------------------+
void CheckForSignal()
  {
   ...
   //--- Deteksi kondisi dasar (raw condition dari Phase 1)
   bool cond_bullish_trend  = false;
   bool cond_bearish_trend  = false;
   bool cond_oversold_patt  = false;
   bool cond_overbought_patt= false;
   
   //=================================================================
   //=== NODE 1: DETEKSI TREND (EMA + ADX Trend & DI + RSI Non-Ekstrem)
   //=================================================================
   if(InpTradeMode==MODE_FOLLOW_ONLY || InpTradeMode==MODE_BOTH)
     {
      ...
      // Kondisi Bullish Phase 1: harga > EMA, ADX kuat + +DI > -DI, RSI belum Overbought
      if(close1 > ema1 && adx1 >= InpADX_Strength && adx_buy_dir && rsi1 < InpRSI_Overbought)
        {
         cond_bullish_trend = true;
        }
      
      // Kondisi Bearish Phase 1: harga < EMA, ADX kuat + -DI > +DI, RSI belum Oversold
      if(close1 < ema1 && adx1 >= InpADX_Strength && adx_sell_dir && rsi1 > InpRSI_Oversold)
        {
         cond_bearish_trend = true;
        }
     }
   
   //=================================================================
   //=== NODE 2: DETEKSI POLA EKSTREM (RSI Ekstrem + Konfirmasi Bar 1-2)
   //=================================================================
   if(InpTradeMode==MODE_REVERSAL_ONLY || InpTradeMode==MODE_BOTH)
     {
      // Kondisi Oversold + Pola Bullish (di Phase 1 memicu BUY)
      if(rsi1 <= InpRSI_Oversold)
        {
         bool bullish = false;
         if(InpUsePinBar && IsPinBarBullish())       bullish = true;
         if(InpUseEngulfing && IsEngulfingBullish()) bullish = true;
         if(bullish) cond_oversold_patt = true;
        }
      
      // Kondisi Overbought + Pola Bearish (di Phase 1 memicu SELL)
      if(rsi1 >= InpRSI_Overbought)
        {
         bool bearish = false;
         if(InpUsePinBar && IsPinBarBearish())       bearish = true;
         if(InpUseEngulfing && IsEngulfingBearish()) bearish = true;
         if(bearish) cond_overbought_patt = true;
        }
     }
   
   //=================================================================
   //=== LOGIKA PEMBALIKAN (CONTRARIAN / INVERSE MAPPING)          ===
   //=== Bullish -> Eksekusi SELL | Bearish -> Eksekusi BUY        ===
   //=================================================================
   bool do_buy  = false;
   bool do_sell = false;

   // Jika kondisi pasar Bearish / Overbought -> Reversal Engine memicu BUY
   if(cond_bearish_trend || cond_overbought_patt)
     {
      do_buy = true;
      if(cond_bearish_trend && cond_overbought_patt)
         last_signal_desc = "REVERSAL BUY (Fade Bearish Trend & Overbought Pattern)";
      else if(cond_bearish_trend)
         last_signal_desc = "REVERSAL BUY (Fade Bearish Trend [Harga < EMA])";
      else
         last_signal_desc = "REVERSAL BUY (Inverse Overbought Dip Pattern)";

      if(InpDebugLog) Print("REVERSAL TRIGGER: BUY triggered (Inverted from Bearish/Overbought)");
     }

   // Jika kondisi pasar Bullish / Oversold -> Reversal Engine memicu SELL
   if(cond_bullish_trend || cond_oversold_patt)
     {
      do_sell = true;
      if(cond_bullish_trend && cond_oversold_patt)
         last_signal_desc = "REVERSAL SELL (Fade Bullish Trend & Oversold Pattern)";
      else if(cond_bullish_trend)
         last_signal_desc = "REVERSAL SELL (Fade Bullish Trend [Harga > EMA])";
      else
         last_signal_desc = "REVERSAL SELL (Inverse Oversold Bounce Pattern)";

      if(InpDebugLog) Print("REVERSAL TRIGGER: SELL triggered (Inverted from Bullish/Oversold)");
     }

   //=================================================================
   //=== NODE 3: ML/ONNX Inference Filter                          ===
   //=================================================================
   if(InpUseMLFilter)
     {
      if(do_buy)
        {
         // Uji probabilitas untuk posisi BUY yang akan dibuka (signal_type = 1)
      ...
      if(do_sell)
        {
         // Uji probabilitas untuk posisi SELL yang akan dibuka (signal_type = 2)
...
void OpenPosition(ENUM_ORDER_TYPE type)
  {
   ...
   bool res = false;
   if(type == ORDER_TYPE_BUY)
      res = trade.Buy(lot, trade_symbol, price, sl, tp, "XAU Reversal BUY");
   else if(type == ORDER_TYPE_SELL)
      res = trade.Sell(lot, trade_symbol, price, sl, tp, "XAU Reversal SELL");
...
void ManagePositions()
  {
   ...
   if(pos_type == POSITION_TYPE_BUY && cur_price <= last_entry - dist_pts)
      should_average = true;  // Posisi BUY floating minus jika harga turun -> averaging BUY lagi
   else if(pos_type == POSITION_TYPE_SELL && cur_price >= last_entry + dist_pts)
      should_average = true;  // Posisi SELL floating minus jika harga naik -> averaging SELL lagi
   ...
   bool res = false;
   string comment = "XAU Rev Averaging L" + IntegerToString(current_level);
